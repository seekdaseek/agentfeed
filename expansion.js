...
const EXP = [
  ...
  { name: 'get_liq_history', route: 'GET /api/liq-history', usd: 0.05,
    desc: 'HISTORICAL liquidation tape, time-bucketed: total/long/short USD, prints, biggest print per bucket. Any USDT perp or the whole recorded universe, up to 7 days back. Bybit is the only complete liq tape in crypto and no exchange publishes history of it — this data exists nowhere else.',
  ...
  { name: 'get_funding_extremes', route: 'GET /api/funding-extremes', usd: 0.02,
    desc: 'Most crowded trades across every Bybit USDT perp: top most-positive and most-negative funding with annualized %, 24h price move and OI. Crowded shorts = squeeze candidates.',
  ...
  { name: 'get_oi_spike_scan', route: 'GET /api/oi-spike-scan', usd: 0.02,
    desc: 'Abnormal open-interest jumps across every Bybit USDT perp vs a 30min+ baseline — where new leverage is piling in, with funding and price context. Squeeze/flush precursor screener.',
  ...
  { name: 'get_top_movers', route: 'GET /api/top-movers', usd: 0.01,
    desc: '24h top gainers and losers across every Bybit USDT perp with a liquidity floor, funding attached. The "what moved" screener.',
